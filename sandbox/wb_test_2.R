...
lon <- as.vector(lonm)
lat <- as.vector(latm)
u <- as.vector(u) # note making u,v into vectors
v <- as.vector(v) # note making u,v into vectors

# Step 2: create map underlay
if (!interactive()) {
    png("wb_test_2.png", unit = "in", width = 7, height = 5, res = 200)
}
library(oce)
data(coastlineWorld)
par(mar = rep(2, 4))
...
# FIXME: permit lon and lat to be vectors, matching to matrices u and v.
mapDirectionFieldBarbs(lon, lat, u, v, step = 10, length = 5, debug = TRUE)
